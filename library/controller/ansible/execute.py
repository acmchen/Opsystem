...
class Exec_Tasks(Base):
    ...
    def adhoc(self, username, name, vault_password, pattern_list, module_name, module_args, describe):
        ...
        if len(result)==4:
            host_name = result[3]
            inve_protect_content = [inve_protect_content, host_name]
        ...
        from devops.tasks import ansible_send_adhoc

        result = ansible_send_adhoc.delay(name, username, option_dict, describe, inve_protect_content)
        self.logger.info('用户' + username +'构建名为' + name + ' 的ansible临时任务成功')
        return (True, '')
    ...
    def playbook(self, username, name, vault_password, pattern_list, uuidstr, describe):
        ...
        if yaml_result[0] :
            ...
            return (False, '读取yaml数据时出错，' + yaml_result[1])
        # 替换掉yaml文件中的hosts
        ...
        from devops.tasks import ansible_send_playbook
