...
from library.controller import Base
...
class Manager_Inventory(Base):
    def get_init_para(self, username):
        ...
        self.inve_rediskey = 'devops:' + username + ':inventory'
        self.group_rediskey = 'devops:' + username + ':inventory:group'
        self.invetemp_rediskey = 'devops:' + username + ':inventory:temp'
    ...
    def add_get(self, username):
        ...
        html_code = ''
        range_list = ['name','ssh_host','os','os_ver','group','ssh_port','ssh_user','ssh_pass','sudo_pass','sudo_exec','ssh_private_key_file','shell_type','connection','python_interpreter']
        for key in range_list:
            value = self.host_name_dict[key]
            ...
            if isinstance(default_value, (list, tuple)) :
                if key != 'group' :
                    ...
                    option_html = '<input type="text" name="group_add" id="group_add" class="" placeholder="">\n</br></br>'
                    option_html = option_html + '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'
                    option_html = option_html + '<select multiple id="' + key + '" name="' + key + '"style="width:170px;">\n</br>'
                    for k in default_value :
                        option_html = option_html + '<option id="' + k + '" value="' + k + '">' + k + '</option>\n</br>'
                    option_html = option_html + '</select>\n</br>\n</br>'
            ...
            if not isinstance(default_value, (list, tuple)) :
                if key in self.reqfield_list :
                    html_code = html_code + value + '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="text" name="' + key + '" class="" value="' + default_value + '" placeholder="' + default_value + '" required="length[2~16]">\n</br>\n</br>\n'
                else :
                    html_code = html_code + value + '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="text" name="' + key + '" class="" value="' + default_value + '" placeholder="' + default_value + '">\n</br>\n</br>\n'
            else :
    ...
    def summary(self, username):
        ...
        if result[0] :
            title_list = []
            display_list = ('name', 'ssh_host', 'os', 'os_ver', 'group')
    ...
    def status_view_data(self,host_status):
        if host_status == '':
            return host_status

        status_list = host_status.split('\n')

        loadav = status_list[0].split('load average:')[1].strip()
        tasks_dict = {}
        tasks = re.split(r":|,",status_list[1])
        for ta in tasks[1:]:
            tmp = ta.strip().split(' ')
            tasks_dict[tmp[1]] = tmp[0]
        cpus_dict = {}
        cpu = re.split(r":|,",status_list[2])
        for c in cpu[1:]:
            if '%' in c:
                tmp = c.strip().split('%')
            else:
                tmp = c.strip().split(' ')
            cpus_dict[tmp[1]] = tmp[0]
        mem_dict = {}
        mem = re.split(r":|,",status_list[3])
        for m in mem[1:]:
            if 'k' in m:
                tmp = m.strip().split('k ')
            else:
                tmp = m.strip().split(' ')
            mem_dict[tmp[1]] = tmp[0]
        swap_dict = {}

        swap = re.split(r":|,",status_list[4])
        ...
        return view_data        
    def detail_host_status(self,username, vault_password, host_name, group_list):
        from library.controller.ansible.execute import Exec_Tasks
        name = host_name + '_now_status'
        pattern_list = [group_list,{'host_name':host_name}]
        module_name = 'shell'
        argv = 'top -bn1 | head -5'
        describe = 'get ' + host_name + ' now_status'
    ...
    def detail(self, username, vault_password, name):
        ...
        if result[0] :
            ...
            return result
        self.detail_host_status(username,vault_password,name,host_dict['group'])
        range_list = ['name','ssh_host','os','os_ver','group','ssh_port','ssh_user','ssh_pass','sudo_pass','sudo_exec','ssh_private_key_file','shell_type','connection','python_interpreter']
        ...
        host_dict['now_status'] = self.status_view_data(host_dict['now_status'])
        host_data = []
        for key in range_list :
            zh = self.host_name_dict[key]
            zh = zh.replace(' ', '')
            zh = zh.replace('&nbsp;', '')
            if key in host_dict :
                host_data.append((zh,host_dict[key]))
                    
        return (True, host_data,host_dict['now_status'])
    ...
    def get_grouplist(self, username, force=False):
        ...
        if not new_group_list:
            new_group_list = new_group_list + ['all']
    ...
    def write_file(self, username, vault_password, group_list=[]):
        ...
        host_name = ''
        if len(group_list)>1 and isinstance(group_list[1],dict):
            host_name = group_list[1]['host_name']
        ...
        inve_file = '/dev/shm/devops/ansible/inventory_' + random_str()

        result = write_file(inve_file , 'w' , content_str)
        if result[0] :
            self.logger.info('将用户' + username + '主机按照ansible的hosts.conf格式写入临时文件成功')
            return (True, inve_file, protect_content_str, host_name)
        else :
