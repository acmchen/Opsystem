...
class Manager_Uri():
    ...
    def get_nav(self, username, level=1, force=False):
        
        '''
        获取左侧栏信息
        '''
        
        redis_key = 'devops:' + username + ':uri:nav'
        if force:
        ...
        nav_str = self.gen_nav(username,level)
        set_dict = {
            'name' : redis_key,
            'value' : nav_str,
            'ex':self.expiretime
    ...
    def gen_nav(self, username,user_level):
        ...
        for uri, value in self.uridict.items() :
            if value['nav'] and user_level >= value['user_level']:
                nav_dict[uri] = value
