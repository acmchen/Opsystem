...
def write_random_file(data):
    for i in range(10) :
        temp_file = '/dev/shm/devops/' + random_str(ranlen=30) + str(i) + '_' + str(time.time())
        result = write_file(temp_file, 'w', data)
        if result[0] :
            return (True, temp_file)
        
    return (False, '临时文件无法写入/dev/shm/devops/')
...
def get_filetype(this_path):
    ...
    # /opt/devops/example/ansible/playbook/ntpdate.yaml: text/plain
