...
class Logging_Mongo():
    ...
    def write(self, content, oper, log_dict={}):
        ...
        if isinstance(content, dict) :
            if 'hostname' in log_dict :
                if 'detail' in content and content['detail']:
                    print (content['detail'])
                    msg_dict = {'now_status':content['detail'][log_dict['hostname']]['stdout']}
                flag = 1
            else:
        ...
        if flag == 0 :
            ...
            result = self.mongoclient.find(collect, condition_dict={'name': log_dict['hostname']})
            data_new = result[1][0]
            data_new['now_status'] = msg_dict['now_status']
            update_dict = {
                'collect': collect,
                'data': data_new
            }
            print (update_dict)
            result = self.mongoclient.update({'name':log_dict['hostname']},update_dict, addtime=True)
        else:
            return(False,False)
