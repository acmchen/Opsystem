...
from library.controller.ansible.options import Manager_Option
from library.controller.ansible.report import Manager_Report
from library.controller.sysadmin.inventory import Manager_Inventory
from library.controller.sysadmin.uri import Manager_Uri
from library.controller.sysadmin.user import Manager_User
...
class Base():
    ...
    def _is_login(self, request):
        ...
        try :
            ...
            try :
                self.nav_html = self.uri_api.get_nav(username,level)
            except :
                self.nav_html = ''
...
class Login(Base):
    def login(self, request):
        ...
        result = self.usermanager_api.get_userinfo(force=True,username=adminuser)
        if not result :
        ...
        if request.method == 'GET' :
            ...
            if form.is_valid():
                ...
                if not result[0] :
                    ...
                    request.session['level'] = result[2]
                    return HttpResponseRedirect(reverse('index'))
