...
from devops.forms import Form_Login
from devops.views import Base
...
class User(Base):
    def add(self, request):
        ...
        if request.method == 'GET' :
            ...
            if form.is_valid():
                post_key_list = ['username', 'name', 'contact', 'password', 'password-confirm', 'vaultpassword', 'vaultpassword-confirm','level']
                user_mess_dict = {}
                for key in post_key_list :
                    user_mess_dict[key] = request.POST.get(key)
                
                user_mess_dict['creater'] = creater
                user_mess_dict['level'] = int(user_mess_dict['level'])
    ...
    def detail(self, request):
        ...
        result = self.usermanager_api.detail(username)
        if result[0] :
            self.logger.info(self.username + u' 查看用户' + username + u'的详细数据成功')
            return render(request, 'user_detail.html', {'username':username, 'detail_dict':result[2] , 'login_user':login_name, 'nav_html':self.nav_html, 'lately_whereabouts':self.latelywhere_html})
        else :
